			...
			if(err || rows.length == 0) {
				...
				for(i = 0; i < rows.length; i++) {
					if(rows[i][fields[0].name] != '_migrations') {
						...
						tables[keys[keys.length - 1]].columns = [];
						tables[keys[keys.length - 1]].defaults = [];
					}
				}
			}
			...
			db.query('SHOW COLUMNS FROM ??', item, function(err, rows, fields) {
				...
				if(err) {
					...
					for(i = 0; i < rows.length; i++) {
						...
						tables[item].columns.push(rows[i].Field);
						if(rows[i].Default !== null) {
							tables[item].defaults.push(rows[i].Default);
						} else if(rows[i].Type.indexOf('int(') != -1) {
							tables[item].defaults.push(0);
						} else {
							tables[item].defaults.push('');
						}
